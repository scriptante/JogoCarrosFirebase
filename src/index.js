...
table.addEventListener("click", (event) => {
  ...
  // implementa destaque ao item
  if (event.target.classList.contains("destaque")) {
    const id = event.target.parentElement.parentElement.getAttribute("data-id");
    ...
    db.collection("carros")
      ...
      .then((doc) => {
        db.collection("carros")
          .doc(id)
          .update({ destaque: !doc.data().destaque })
          .then(() => {
            console.log("Ok! Item em destaque");
          });
      })
      .catch(function (error) {
        console.error("Error Spotlight document: ", error);
      });
    table.innerHTML = "";
  }
});
